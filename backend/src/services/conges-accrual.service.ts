 ...
 */
function moisDebutAccrual(dateEntree: Date | null, annee: number): number {
  if (!dateEntree) return 1;
  const anneeEntree = dateEntree.getFullYear();
  if (anneeEntree < annee) return 1;
  if (anneeEntree > annee) return 13;
 ...
 */
export async function accruerEmployePourMois(
  employeId: string,
  annee: number,
  mois: number,
  auteurId?: string
 ...
 */
export async function rattraperAccrualEmploye(
  employeId: string,
 ...
 */
export async function rattraperAccrualTousEmployes(): Promise<number> {
