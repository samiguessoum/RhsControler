...
export const facturationStatsController = {
    ...
    async getTVASummary(req, res, next) {
        try {
    ...
    async getMarges(req, res, next) {
        try {
            ...
            // Totaux
            const totaux = marges.reduce((acc, m) => ({
    ...
    async getCommandesFacturables(req, res, next) {
        try {
    ...
    // Flux de trésorerie
    async getTresorerie(req, res, next) {
        try {
            ...
            for (let month = 0; month < 12; month++) {
                ...
                const decaissements = (fournis._sum.montant || 0) +
            ...
            const totalDecaissements = (paiementsFournisseurs._sum.montant || 0) + (paiementsCharges._sum.montant || 0) + decaissementsDivers;
            res.json({
                annee: year,
                encaissements: {
    ...
    // Factures en retard
    async getFacturesEnRetard(req, res, next) {
        try {
    ...
    async getMensuel(req, res, next) {
        try {
            const { annee } = req.query;
            const year = annee ? parseInt(annee) : new Date().getFullYear();
            const MOIS = ['Jan', 'Fév', 'Mar', 'Avr', 'Mai', 'Jun', 'Jul', 'Aoû', 'Sep', 'Oct', 'Nov', 'Déc'];
            const mois = await Promise.all(Array.from({ length: 12 }, async (_, m) => {
    ...
    async getAnneesDisponibles(_req, res, next) {
        try {
        ...
        try {
            ...
            const moisDetails = Array.from({ length: 12 }, (_, i) => {
                ...
                const paiServMois = paiementsServices.filter(p => {
                    const d = new Date((p.dateEncaissement ?? p.datePaiement));
                    return d.getMonth() === i;
                });
...
//# sourceMappingURL=facturation-stats.controller.js.map