...
async function assertFieldInterventionAccess(req, fieldInterventionId) {
    if (req.user.role !== 'EQUIPE')
        return;
    if (!req.user.employeId)
        throw new AppError(403, 'Compte non lié à une fiche employé');
    const assigned = await prisma.fIApplicateur.findFirst({
        where: { fieldInterventionId, employeId: req.user.employeId },
        select: { id: true },
    });
    if (!assigned)
        throw new AppError(403, "Vous n'êtes pas affecté à cette intervention");
}
export const fieldInterventionController = {
    ...
    async list(req, res, next) {
        try {
            ...
            if (dateFrom || dateTo) {
                ...
                if (dateFrom)
                    where.dateIntervention.gte = new Date(dateFrom);
                if (dateTo)
                    where.dateIntervention.lte = new Date(dateTo);
            }
    ...
    async get(req, res, next) {
        try {
            ...
            if (!fi)
                throw new AppError(404, 'Intervention terrain introuvable');
            res.json(fi);
        }
        catch (err) {
            next(err);
    ...
    async create(req, res, next) {
        try {
            ...
            const { siteId, clientId, zoningVersionId, type, dateIntervention, heureDebut, heureFin, commentaire, applicateurIds, contratId, } = req.body;
            ...
            const fi = await prisma.fieldIntervention.create({
                data: {
                    ...
                    applicateurs: applicateurIds?.length
                        ? { create: applicateurIds.map((eid) => ({ employeId: eid })) }
                        : undefined,
                },
                include: {
    ...
    async update(req, res, next) {
        try {
            ...
            if (!existing)
                throw new AppError(404, 'Intervention introuvable');
                ...
                if (applicateurIds !== undefined) {
                    ...
                    if (applicateurIds.length > 0) {
                        await tx.fIApplicateur.createMany({
                            data: applicateurIds.map((eid) => ({ fieldInterventionId: id, employeId: eid })),
                        });
                    }
                }
    ...
    async validate(req, res, next) {
        try {
            ...
            if (!existing)
                throw new AppError(404, 'Intervention introuvable');
            if (existing.statut !== 'SUBMITTED')
                throw new AppError(400, "L'intervention doit être soumise avant validation");
            const updated = await prisma.fieldIntervention.update({
                where: { id },
    ...
    async cancel(req, res, next) {
        try {
            ...
            if (existing.statut === 'VALIDATED')
                throw new AppError(400, 'Une intervention validée ne peut pas être annulée');
    ...
    async upsertControls(req, res, next) {
        try {
            const { id } = req.params;
            await assertFieldInterventionAccess(req, id);
            const { controls } = req.body;
            if (!Array.isArray(controls))
                throw new AppError(400, 'controls doit être un tableau');
    ...
    async upsertProducts(req, res, next) {
        try {
            const { id } = req.params;
            await assertFieldInterventionAccess(req, id);
            const { products } = req.body;
            if (!Array.isArray(products))
                throw new AppError(400, 'products doit être un tableau');
            await prisma.$transaction(async (tx) => {
    ...
    async getSiteAnalytics(req, res, next) {
        try {
            ...
            const fiWhere = { siteId, statut: { in: ['SUBMITTED', 'VALIDATED'] } };
            if (zoningVersionId)
                fiWhere.zoningVersionId = zoningVersionId;
            if (dateFrom || dateTo) {
                fiWhere.dateIntervention = {};
                if (dateFrom)
                    fiWhere.dateIntervention.gte = new Date(dateFrom);
                if (dateTo)
                    fiWhere.dateIntervention.lte = new Date(dateTo);
            }
            ...
            for (const fi of interventions) {
                for (const ctrl of fi.controls) {
                    ...
                    if (!deviceStats[key])
                        deviceStats[key] = { interventionCount: 0, statusCodes: [], insectTotal: 0 };
                    deviceStats[key].interventionCount++;
                    if (ctrl.statusCode)
                        deviceStats[key].statusCodes.push(ctrl.statusCode);
    ...
    async listSiteFieldReports(req, res, next) {
        try {
            const { siteId } = req.params;
    ...
    async generateSiteFieldReport(req, res, next) {
        try {
            const { siteId } = req.params;
            const { dateFrom, dateTo } = req.body;
            if (!dateFrom || !dateTo)
                throw new AppError(400, 'dateFrom et dateTo sont requis');
            const { fieldReportService } = await import('../services/field-report.service.js');
            const report = await fieldReportService.generateSiteHistoryReport(siteId, new Date(dateFrom), new Date(dateTo), req.user.id);
    ...
    async downloadFieldReport(req, res, next) {
        try {
            const { id } = req.params;
            const report = await prisma.fieldReport.findUnique({ where: { id } });
            if (!report?.xlsxPath)
                throw new AppError(404, 'Rapport introuvable');
    ...
    async listSiteDocuments(req, res, next) {
        try {
            const { siteId } = req.params;
    ...
    async createSiteDocument(req, res, next) {
        try {
            const { siteId } = req.params;
            const { titre, type, filename, path, size, mimeType, date, annee, commentaire } = req.body;
            if (!titre || !filename || !path)
                throw new AppError(400, 'titre, filename et path sont requis');
    ...
    async deleteSiteDocument(req, res, next) {
        try {
            const { id } = req.params;
...
//# sourceMappingURL=field-intervention.controller.js.map