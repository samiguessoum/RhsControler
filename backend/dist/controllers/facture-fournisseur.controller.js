...
async function generateReference(date) {
    ...
    const prefix = settings?.prefixFactureFournisseur || DEFAULT_PREFIX;
    ...
    const offset = settings?.offsetFactureFournisseur || 0;
...
function computeTotals(lignes, remiseGlobalPct, remiseGlobalMontant) {
...
async function buildLignes(lignes) {
...
function determineStatut(totalTTC, totalPaye, dateEcheance) {
...
export const factureFournisseurController = {
        ...
        try {
            ...
            if (search) {
                where.OR = [
                    ...
                    { refFournisseur: { contains: search, mode: 'insensitive' } },
                    { fournisseur: { nomEntreprise: { contains: search, mode: 'insensitive' } } },
                ];
            }
            if (dateDebut || dateFin) {
                where.dateFacture = {};
                if (dateDebut)
                    where.dateFacture.gte = new Date(dateDebut);
                if (dateFin)
                    where.dateFacture.lte = new Date(dateFin);
            }
    ...
    async convertirFromCommande(req, res, next) {
        try {
    ...
    async exportPDF(req, res, next) {
        try {
            ...
            const pdfBuffer = await generateFactureFournisseurPDF(facture);
...
//# sourceMappingURL=facture-fournisseur.controller.js.map