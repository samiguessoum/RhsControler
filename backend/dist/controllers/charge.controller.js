...
function determineStatut(montantTTC, montantPaye) {
...
export const chargeController = {
    ...
    async list(req, res, next) {
        try {
            const { search, typeCharge, categorie, statut, fournisseurId, dateDebut, dateFin, page = '1', limit = '50', } = req.query;
            const where = {};
            if (typeCharge)
                where.typeCharge = typeCharge;
            if (categorie)
            ...
            if (dateDebut || dateFin) {
                where.dateCharge = {};
                if (dateDebut)
                    where.dateCharge.gte = new Date(dateDebut);
                if (dateFin)
                    where.dateCharge.lte = new Date(dateFin);
            }
