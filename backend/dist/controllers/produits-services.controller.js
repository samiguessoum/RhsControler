...
export const produitsServicesController = {
    ...
    async list(req, res, next) {
        try {
            const { search, type, categorieId, fournisseurId, actif, stockBas, enVente, enAchat, page = '1', limit = '50' } = req.query;
            const where = {};
            if (actif !== undefined) {
    ...
    async create(req, res, next) {
        try {
            const { reference, codeBarres, nom, description, descriptionLongue, type, nature, unite, uniteAchat, ratioUnites, prixVenteHT, tauxTVA, prixAchatHT, margeParDefaut, aStock, modeGestion, quantite, stockMinimum, stockMaximum, lotSuivi, dlcSuivi, dureeService, fournisseurId, fournisseursDefaut, delaiLivraison, marque, modele, poids, longueur, largeur, hauteur, compteVente, compteAchat, notePublique, notePrivee, urlExterne, enVente, enAchat, categorieIds, entrepotInitialId, } = req.body;
            ...
            if (fournisseursDefaut && fournisseursDefaut.length > 0) {
                await prisma.produitFournisseurDefaut.createMany({
                    data: fournisseursDefaut.map((fd) => ({
    ...
    async update(req, res, next) {
        try {
            const { id } = req.params;
            const { reference, codeBarres, nom, description, descriptionLongue, type, nature, unite, uniteAchat, ratioUnites, prixVenteHT, tauxTVA, prixAchatHT, margeParDefaut, aStock, modeGestion, stockMinimum, stockMaximum, lotSuivi, dlcSuivi, dureeService, fournisseurId, fournisseursDefaut, delaiLivraison, marque, modele, poids, longueur, largeur, hauteur, compteVente, compteAchat, notePublique, notePrivee, urlExterne, enVente, enAchat, actif, categorieIds, } = req.body;
     ...
     * GET /api/entrepots
     */
    async listEntrepots(req, res, next) {
        try {
     ...
     */
    async getEntrepot(req, res, next) {
        try {
            const { id } = req.params;
     ...
     * POST /api/entrepots
     */
    async createEntrepot(req, res, next) {
        try {
     ...
     */
    async updateEntrepot(req, res, next) {
        try {
            const { id } = req.params;
     ...
     */
    async deleteEntrepot(req, res, next) {
        try {
            const { id } = req.params;
     ...
     */
    async listPrixFournisseurs(req, res, next) {
        try {
     ...
     */
    async createPrixFournisseur(req, res, next) {
        try {
     ...
     */
    async updatePrixFournisseur(req, res, next) {
        try {
            const { id } = req.params;
     ...
     */
    async deletePrixFournisseur(req, res, next) {
        try {
            const { id } = req.params;
            const existing = await prisma.prixFournisseur.findUnique({ where: { id } });
            if (!existing) {
     ...
     */
    async listPrixClients(req, res, next) {
        try {
     ...
     */
    async createPrixClient(req, res, next) {
        try {
     ...
     */
    async updatePrixClient(req, res, next) {
        try {
            const { id } = req.params;
     ...
     */
    async deletePrixClient(req, res, next) {
        try {
            const { id } = req.params;
            const existing = await prisma.prixClient.findUnique({ where: { id } });
            if (!existing) {
     ...
     */
    async createMouvementProduitService(req, res, next) {
        try {
            const { id } = req.params;
    ...
    async getStats(req, res, next) {
        try {
            const [totalProduits, totalServices, produitsActifs, servicesActifs, totalCategories, totalEntrepots,] = await Promise.all([
     ...
     */
    async uploadFicheTechnique(req, res, next) {
        try {
            const { id } = req.params;
            if (!req.file) {
     ...
     */
    async deleteFicheTechnique(req, res, next) {
        try {
            const { id } = req.params;
            const produit = await prisma.produitService.findUnique({
                where: { id },
...
//# sourceMappingURL=produits-services.controller.js.map