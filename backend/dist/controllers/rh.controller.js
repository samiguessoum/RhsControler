...
import { startOfYear, endOfYear, startOfWeek, isFriday, isSaturday, format } from 'date-fns';
import logger from '../lib/logger.js';
import { AppError } from '../lib/errors.js';
import { accruerEmployePourMois } from '../services/conges-accrual.service.js';
async function recalculerRecuperation(employeId, annee) {
...
export const rhController = {
    ...
    async listConges(req, res, next) {
        try {
            ...
            if (dateDebut || dateFin) {
                ...
                if (dateFin)
                    where.dateDebut.lte = new Date(dateFin);
            }
     ...
     */
    async createConge(req, res, next) {
        try {
     ...
     */
    async approuverConge(req, res, next) {
        try {
            const { id } = req.params;
     ...
     */
    async annulerConge(req, res, next) {
        try {
            const { id } = req.params;
     ...
     */
    async crediterMois(req, res, next) {
        try {
            ...
            for (const emp of employes) {
                ...
                if (emp.dateEntree) {
                    ...
                    if (anneeEntree > anneeNum || (anneeEntree === anneeNum && moisEntree > moisNum))
                        continue;
                }
     ...
     */
    async ajusterSolde(req, res, next) {
        try {
     ...
     */
    async cloturerAnnee(req, res, next) {
        try {
     ...
     */
    async listMouvements(req, res, next) {
        try {
     ...
     */
    async listWeekendTravailles(req, res, next) {
        try {
     ...
     */
    async createWeekendTravaille(req, res, next) {
        try {
     ...
     */
    async deleteWeekendTravaille(req, res, next) {
        try {
            const { id } = req.params;
     ...
     */
    async updateSolde(req, res, next) {
        try {
     ...
     */
    async getEmployeRecap(req, res, next) {
        try {
            const { id } = req.params;
    ...
    async getDashboard(req, res, next) {
        try {
            ...
            const [congesEnAttente, congesEnCours, employesEnConge, totalEmployes,] = await Promise.all([
     ...
     */
    async listRecuperations(req, res, next) {
        try {
            ...
            const whereEmploye = employeId ? { id: employeId } : {};
     ...
     */
    async accorderRecuperation(req, res, next) {
        try {
     ...
     */
    async deleteRecuperationAccordee(req, res, next) {
        try {
            const { id } = req.params;
