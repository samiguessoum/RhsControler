...
export const zoningController = {
    ...
    async getVersion(req, res, next) {
        try {
            ...
            if (!version)
                throw new AppError(404, 'Version de zoning introuvable');
            res.json(version);
        }
        catch (err) {
            next(err);
    ...
    async updateVersion(req, res, next) {
        try {
            ...
            if (!existing)
                throw new AppError(404, 'Version introuvable');
    ...
    async deleteVersion(req, res, next) {
        try {
            ...
            if (existing.statut !== 'DRAFT')
                throw new AppError(400, 'Seules les versions DRAFT peuvent être supprimées');
    ...
    async deleteZone(req, res, next) {
        try {
            ...
            if (hasDevices > 0)
                throw new AppError(400, 'Impossible de supprimer une zone avec des dispositifs');
    ...
    async createDevice(req, res, next) {
        try {
            ...
            if (!type || !displayNumber)
                throw new AppError(400, 'type et displayNumber sont requis');
            const zone = await prisma.zone.findUnique({ where: { id: zoneId } });
            if (!zone)
                throw new AppError(404, 'Zone introuvable');
    ...
    async deleteDevice(req, res, next) {
        try {
            ...
            if (hasControls > 0)
                throw new AppError(400, 'Impossible de supprimer un dispositif avec des contrôles enregistrés');
    ...
    async listDevices(req, res, next) {
        try {
            ...
            const where = { zoningVersionId: versionId };
            if (type)
                where.type = type;
            if (zoneId)
                where.zoneId = zoneId;
    ...
    async listControlStatuses(req, res, next) {
        try {
    ...
    async createControlStatus(req, res, next) {
        try {
            const { code, label, description, color, ordre } = req.body;
            if (!code || !label)
                throw new AppError(400, 'code et label sont requis');
    ...
    async updateControlStatus(req, res, next) {
        try {
            const { id } = req.params;
