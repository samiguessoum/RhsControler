...
export const settingsController = {
    ...
    async updateSettings(req, res, next) {
        try {
            ...
            nomEntreprise, formeJuridique, logoPath, logoCarrePath, 
            // Coordonnées
            adresse, codePostal, ville, pays, telephone, fax, email, siteWeb, 
            // Informations légales
            rc, nif, ai, nis, nin, capitalSocial, compteBancaire, rib, banque, 
            // Paramètres commerciaux
            devisePrincipale, tauxTVADefaut, 
            // Préfixes de numérotation - Documents vente
            prefixDevis, prefixCommande, prefixFacture, prefixAvoir, 
            // Préfixes de numérotation - Documents achat
            prefixCommandeFournisseur, prefixFactureFournisseur, prefixCharge, 
            // Préfixes de numérotation - Tiers
            prefixClient, prefixFournisseur, prefixProspect, 
            // Préfixes de numérotation - Autres
            prefixProduit, prefixService, 
            // Format numérotation
            longueurNumero, inclureAnnee, separateur, 
            // Décalages de numérotation
            offsetDevis, offsetCommande, offsetFacture, offsetAvoir, offsetCommandeFournisseur, offsetFactureFournisseur, offsetCharge, offsetClient, offsetFournisseur, offsetProspect, 
            // RH - Congés
            modeAllocationConges, joursCongesAnnuels, joursCongesMensuels, } = req.body;
            ...
            const filteredData = Object.fromEntries(Object.entries(data).filter(([, v]) => v !== undefined));
            if (settings) {
                // Mettre à jour
    ...
    async uploadLogoCarre(req, res, next) {
        try {
            if (!req.file) {
                return res.status(400).json({ error: 'Aucun fichier fourni' });
            }
