...
export const dashboardController = {
     ...
     */
    async statsExtended(req, res, next) {
        try {
            ...
            // Stats de base
            const [aPlanifier, enRetard, controles30j, contratsEnAlerte, ponctuelAlerte, contratsAnnuelsFinProche, 
            // Stats du jour
            interventionsAujourdhui, realiseeAujourdhui, 
            // Stats du mois
            totalMois, realiseeMois, annuleeMois, 
            // Stats mois précédent pour tendance
            totalMoisPrecedent, realiseeMoisPrecedent, 
            // Totaux
            totalClients, totalContrats, 
            // Prochaines 7 jours par jour
            interventionsParJour,] = await Promise.all([
            ...
            for (let i = 0; i < 7; i++) {
                ...
                const count = interventionsParJour.find((g) => format(new Date(g.datePrevue), 'yyyy-MM-dd') === jourStr)?._count || 0;
    ...
    async alertes(req, res, next) {
        try {
            ...
            const annuelFinProcheAlertes = contratsAnnuelsFinProche.map((c) => ({
     ...
     */
    async employesStats(req, res, next) {
        try {
            const today = startOfDay(new Date());
            ...
            const employesStats = employes.map((employe) => {
                ...
                const missionsThisWeek = interventionsSemaine.filter((i) => i.interventionEmployes.some((ie) => ie.employeId === employe.id));
                ...
                const missionsAVenir = missionsThisWeek.filter((i) => i.statut !== 'REALISEE' && new Date(i.datePrevue) >= today);
                ...
                const missionsLastWeek = interventionsLastWeek.filter((ie) => ie.employeId === employe.id).length;
            ...
            const employesParCharge = [...employesStats].sort((a, b) => b.totalMissionsSemaine - a.totalMissionsSemaine);
            // Employés sans mission cette semaine
            const employesSansMission = employesStats.filter((e) => e.totalMissionsSemaine === 0);
            ...
            const totalMissionsAssignees = interventionsSemaine.filter((i) => i.interventionEmployes.length > 0).length;
            ...
            res.json({
                ...
                stats: {
                    ...
                    moyenneMissionsParEmploye: employes.length > 0
     ...
     */
    async operationsStats(req, res, next) {
        try {
            const today = startOfDay(new Date());
            ...
            for (let i = 0; i < 7; i++) {
                ...
                const jourStr = format(jour, 'yyyy-MM-dd');
                const data = interventionsParJourSemaine.find((g) => format(new Date(g.datePrevue), 'yyyy-MM-dd') === jourStr);
                joursStats.push({
                    date: jourStr,
