...
async function generateCodeTiers(type) {
...
export const tiersController = {
    ...
    async list(req, res, next) {
        try {
            const { search, typeTiers, actif, formeJuridique, secteur, page = '1', limit = '20', } = req.query;
            const where = {};
            ...
            if (secteur) {
                where.secteur = { contains: secteur, mode: 'insensitive' };
            }
            if (search) {
                where.OR = [
                    { code: { contains: search, mode: 'insensitive' } },
                    { nomEntreprise: { contains: search, mode: 'insensitive' } },
                    { nomAlias: { contains: search, mode: 'insensitive' } },
                    { siegeVille: { contains: search, mode: 'insensitive' } },
                    { siegeRC: { contains: search, mode: 'insensitive' } },
                    { siegeNIF: { contains: search, mode: 'insensitive' } },
                    {
                        siegeContacts: {
                            some: {
                                OR: [
        ...
        try {
            ...
            const tiers = await prisma.client.create({
                data: {
                    ...
                    comptesBancaires: {
                        create: data.comptesBancaires?.map((cb, i) => ({
                            libelle: cb.libelle,
                            banque: cb.banque,
                            agence: cb.agence,
    ...
    async update(req, res, next) {
        try {
            ...
            const hasSitesData = Array.isArray(data.sites) && data.sites.some((s) => s?.nom && String(s.nom).trim());
            ...
            if (hasSitesData) {
                const sitesFromInput = data.sites
                    .filter((s) => s?.nom && String(s.nom).trim())
                    .map((s) => ({
                ...
                for (const siteToDelete of sitesToDelete) {
                    try {
                        ...
                        if (!hasRelatedData && !hasInterventions) {
                            ...
                            const reasons = [];
                            if (hasRelatedData)
                                reasons.push('contrat en cours');
                            if (hasInterventions)
                                reasons.push('interventions liées');
            ...
            const sitesNotDeleted = req.sitesNotDeleted || [];
            res.json({
                tiers: updatedTiers,
     ...
     */
    async convertirProspect(req, res, next) {
        try {
            const { id } = req.params;
     ...
     */
    async updateAdresse(req, res, next) {
        try {
            const { id } = req.params;
            const data = req.body;
     ...
     */
    async deleteAdresse(req, res, next) {
        try {
            const { id } = req.params;
     ...
     */
    async addCompteBancaire(req, res, next) {
        try {
            const { id } = req.params;
            const data = req.body;
     ...
     */
    async updateCompteBancaire(req, res, next) {
        try {
            const { id } = req.params;
            const data = req.body;
     ...
     */
    async deleteCompteBancaire(req, res, next) {
        try {
            const { id } = req.params;
     ...
     */
    async addSiteContact(req, res, next) {
        try {
            const { id } = req.params;
            const data = req.body;
     ...
     */
    async updateSiteContact(req, res, next) {
        try {
            const { id } = req.params;
            const data = req.body;
     ...
     */
    async deleteSiteContact(req, res, next) {
        try {
            const { id } = req.params;
     ...
     */
    async listModesPaiement(req, res, next) {
        try {
     ...
     */
    async createModePaiement(req, res, next) {
        try {
     ...
     */
    async listConditionsPaiement(req, res, next) {
        try {
     ...
     */
    async createConditionPaiement(req, res, next) {
        try {
    ...
    async getStats(req, res, next) {
        try {
            const [totalClients, totalFournisseurs, totalProspects, clientsActifs, fournisseursActifs, prospectsActifs,] = await Promise.all([
