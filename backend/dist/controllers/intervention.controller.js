...
export const interventionController = {
    ...
    async list(req, res, next) {
        try {
            ...
            if (dateDebut || dateFin) {
                where.datePrevue = {};
                if (dateDebut)
                    where.datePrevue.gte = startOfDay(parseISO(dateDebut));
                if (dateFin)
                    where.datePrevue.lte = endOfDay(parseISO(dateFin));
            }
     ...
     */
    async aPlanifier(req, res, next) {
        try {
            const { days = '7' } = req.query;
            const interventions = await planningService.getAPlanifier(parseInt(days));
     ...
     */
    async startFieldReport(req, res, next) {
        try {
            const { id } = req.params;
            const intervention = await prisma.intervention.findUnique({
                where: { id },
    ...
    async reporter(req, res, next) {
        try {
            ...
            const intervention = await planningService.reporter(id, req.user.id, new Date(nouvelleDatePrevue), raison);
            res.json({ intervention });
        }
        catch (error) {
     ...
     */
    async exportAttestationPassage(req, res, next) {
        try {
            const { id } = req.params;
     ...
     */
    async getAttestationBody(req, res, next) {
        try {
            const { id } = req.params;
            const { garantieMois, ville } = req.query;
     ...
     */
    async updateAttestationBody(req, res, next) {
        try {
            const { id } = req.params;
     ...
     */
    async exportAttestationGarantie(req, res, next) {
        try {
            const { id } = req.params;
            const { garantieMois, ville } = req.query;
            const data = await attestationService.buildAttestationData(id, {
     ...
     */
    async getAttestationGarantieBody(req, res, next) {
        try {
            const { id } = req.params;
            const { garantieMois, ville } = req.query;
            const result = await attestationService.getBodyConfig(id, {
     ...
     */
    async updateAttestationGarantieBody(req, res, next) {
        try {
            const { id } = req.params;
            const { bodyText, garantieMois, ville } = req.body;
            if (!bodyText || typeof bodyText !== 'string') {
     ...
     */
    async exportAttestationControle(req, res, next) {
        try {
            const { id } = req.params;
            const { garantieMois, ville } = req.query;
            const data = await attestationService.buildAttestationData(id, {
     ...
     */
    async getAttestationControleBody(req, res, next) {
        try {
            const { id } = req.params;
            const { garantieMois, ville } = req.query;
            const result = await attestationService.getBodyConfig(id, {
     ...
     */
    async updateAttestationControleBody(req, res, next) {
        try {
            const { id } = req.params;
            const { bodyText, garantieMois, ville } = req.body;
            if (!bodyText || typeof bodyText !== 'string') {
     ...
     */
    async getLastNotes(req, res, next) {
        try {
