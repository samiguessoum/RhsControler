...
async function generateReference(type, date) {
    const annee = date.getFullYear();
    // Récupérer les paramètres de numérotation
    const settings = await prisma.companySettings.findFirst();
    const prefixField = PREFIX_FIELD_MAP[type];
    const prefixValue = settings ? settings[prefixField] : null;
    ...
    const offsetValue = settings ? settings[offsetField] : null;
...
function computeTotals(lignes, remiseGlobalPct, remiseGlobalMontant, sign = 1) {
...
async function buildLignes(lignes, sign = 1) {
    ...
    return lignes.map((ligne, index) => {
        ...
        const prixUnitaireHT = Number(ligne.prixUnitaireHT ?? produit?.prixVenteHT ?? 0);
...
async function updateFacturePaiementStatus(factureId, tx) {
    const db = tx ?? prisma;
    ...
    const totalEnAttente = facture.paiements
        .filter((p) => p.statut === 'RECU' || p.statut === 'DEPOSE')
        .reduce((sum, p) => sum + p.montant, 0);
    let statut;
...
export const commerceController = {
    ...
    async createDevis(req, res, next) {
        try {
            const data = req.body;
    ...
    async updateDevis(req, res, next) {
        try {
            const { id } = req.params;
            const data = req.body;
    ...
    async deleteDevis(req, res, next) {
        try {
            const { id } = req.params;
    ...
    async validerDevis(req, res, next) {
        try {
            const { id } = req.params;
    ...
    async convertirDevisCommande(req, res, next) {
        try {
            const { id } = req.params;
            const devis = await prisma.devis.findUnique({
                where: { id },
            ...
            const commande = await prisma.commande.create({
                data: {
                    ...
                    statut: 'BROUILLON',
                    pipelineStatut: 'VERIFICATION_STOCK',
    ...
    // ============ COMMANDES ============
    async listCommandes(req, res, next) {
        try {
            const { search, clientId, statut, page = '1', limit = '50' } = req.query;
            const where = {};
            if (clientId)
    ...
    async getCommande(req, res, next) {
        try {
            const { id } = req.params;
    ...
    async validerCommande(req, res, next) {
        try {
            const { id } = req.params;
    ...
    async convertirCommandeFacture(req, res, next) {
        try {
            const { id } = req.params;
            const commande = await prisma.commande.findUnique({
                where: { id },
    ...
    // ============ FACTURES ============
    async listFactures(req, res, next) {
        try {
        ...
        try {
            ...
            const facture = await prisma.$transaction(async (tx) => {
                ...
                if (statut === 'VALIDEE' && factureType === 'FACTURE' && req.user?.id) {
                    const stockResult = await stockService.processFactureValidation(newFacture.id, newFacture.lignes, req.user.id, undefined, // entrepotId - peut être ajouté plus tard
                    tx);
        ...
        try {
            ...
            const facture = await prisma.$transaction(async (tx) => {
                ...
                if (wasValidated && nextType === 'FACTURE' && req.user?.id) {
                    const stockResult = await stockService.processFactureValidation(updatedFacture.id, updatedFacture.lignes, req.user.id, undefined, // entrepotId
                    tx);
                    if (!stockResult.success) {
                        throw new Error(`Erreur stock: ${stockResult.errors.join(', ')}`);
                    }
                ...
                if (existing.statut === 'VALIDEE' && newStatut === 'ANNULEE' && existing.type === 'FACTURE' && req.user?.id) {
                    await stockService.reverseFactureMouvements(existing.id, existing.lignes, req.user.id, undefined, // entrepotId
                    tx);
                }
    ...
    async validerFacture(req, res, next) {
        try {
            const { id } = req.params;
            ...
            const facture = await prisma.$transaction(async (tx) => {
                ...
                if (updated.type === 'FACTURE' && updated.typeDocument !== 'SERVICE' && req.user?.id) {
                    const stockResult = await stockService.processFactureValidation(updated.id, updated.lignes, req.user.id, undefined, tx);
                    if (!stockResult.success) {
    ...
    async listRelances(req, res, next) {
        try {
            const { id } = req.params;
    ...
    async createRelance(req, res, next) {
        try {
            const { id } = req.params;
            const data = req.body;
    ...
    // ============ PAIEMENTS ============
    async createPaiement(req, res, next) {
        try {
            const data = req.body;
            const facture = await prisma.facture.findUnique({
    ...
    async updateStatutCheque(req, res, next) {
        try {
            const { id } = req.params;
            ...
            if (statut === 'DEPOSE')
                updateData.dateDepot = date ? new Date(date) : new Date();
            if (statut === 'ENCAISSE')
                updateData.dateEncaissement = date ? new Date(date) : new Date();
    ...
    async deletePaiement(req, res, next) {
        try {
            const { id } = req.params;
            const paiement = await prisma.paiement.findUnique({ where: { id } });
            if (!paiement) {
    ...
    // ============ EXPORT PDF ============
    async exportDevisPDF(req, res, next) {
        try {
            const { id } = req.params;
            const devis = await prisma.devis.findUnique({
                where: { id },
    ...
    async exportCommandePDF(req, res, next) {
        try {
            const { id } = req.params;
            const commande = await prisma.commande.findUnique({
                where: { id },
            ...
            const pdfBuffer = await generateCommandePDF(commande);
            res.setHeader('Content-Type', 'application/pdf');
    ...
    async exportFacturePDF(req, res, next) {
        try {
            const { id } = req.params;
            const facture = await prisma.facture.findUnique({
                where: { id },
    ...
    // ── Bons de Livraison ──────────────────────────────────────────────────────
    async listBonsLivraison(req, res, next) {
        try {
            const { clientId, commandeId, statut, search, page = '1', limit = '50' } = req.query;
    ...
    async getBonLivraison(req, res, next) {
        try {
            const { id } = req.params;
            ...
            if (!bl)
                return next(new AppError(404, 'Bon de livraison non trouvé'));
            res.json(bl);
        }
        catch (error) {
        ...
        try {
            ...
            if (data.commandeId && data.lignes) {
                await Promise.all(data.lignes
                    ...
                    if (!cmdLigne)
                        throw new AppError(400, `Ligne de commande introuvable: ${l.commandeLigneId}`);
    ...
    async updateBonLivraison(req, res, next) {
        try {
            const { id } = req.params;
            const data = req.body;
            const userId = req.user?.id;
    ...
    async deleteBonLivraison(req, res, next) {
        try {
            const { id } = req.params;
            const userId = req.user?.id;
            const existing = await prisma.bonLivraison.findUnique({ where: { id } });
    ...
    async validerBonLivraison(req, res, next) {
        try {
            const { id } = req.params;
            const existing = await prisma.bonLivraison.findUnique({ where: { id } });
            if (!existing)
    ...
    async livrerBonLivraison(req, res, next) {
        try {
            const { id } = req.params;
            const existing = await prisma.bonLivraison.findUnique({ where: { id } });
            if (!existing)
    ...
    async annulerBonLivraison(req, res, next) {
        try {
            const { id } = req.params;
            const existing = await prisma.bonLivraison.findUnique({ where: { id } });
            if (!existing)
    ...
    async creerBLFromCommande(req, res, next) {
        try {
            ...
            if (!commande)
                return next(new AppError(404, 'Commande non trouvée'));
    ...
    async getCommandeProgressionLivraison(req, res, next) {
        try {
            const { id: commandeId } = req.params;
            const commande = await prisma.commande.findUnique({
                where: { id: commandeId },
    ...
    async exportBonLivraisonPDF(req, res, next) {
        try {
            const { id } = req.params;
            const bl = await prisma.bonLivraison.findUnique({
                where: { id },
    ...
    // ── Pipeline ventes ─────────────────────────────────────────────────────────
    async getPipelineVentes(req, res, next) {
        try {
    ...
    async updatePipelineStatut(req, res, next) {
        try {
            const { id } = req.params;
            ...
            if (!existing)
                return res.status(404).json({ error: 'Commande non trouvée' });
            const data = { pipelineStatut };
            if (livraisonDirect !== undefined)
                data.livraisonDirect = livraisonDirect;
            const commande = await prisma.commande.update({
                where: { id },
                data,
    ...
    async addCommandeToPipeline(req, res, next) {
        try {
            const { id } = req.params;
            const existing = await prisma.commande.findUnique({ where: { id } });
            if (!existing)
    ...
    async removeCommandeFromPipeline(req, res, next) {
        try {
            const { id } = req.params;
            const commande = await prisma.commande.update({
                where: { id },
    ...
    async _checkAndUpdateCommandeStatut(commandeId) {
        if (!commandeId)
            return;
        try {
            const commande = await prisma.commande.findUnique({
            ...
            if (!commande || commande.statut === 'ANNULEE' || commande.statut === 'LIVREE')
                return;
            const dejaLivrees = await prisma.bonLivraisonLigne.groupBy({
                by: ['commandeLigneId'],
                where: {
