...
export const stockService = {
     ...
     */
    async createMouvement(input, tx) {
        const db = tx || prisma;
        try {
     ...
     */
    async processFactureValidation(factureId, lignes, userId, entrepotId, tx) {
        const errors = [];
        const db = tx || prisma;
     ...
     */
    async reverseFactureMouvements(factureId, lignes, userId, entrepotId, tx) {
        const errors = [];
        const db = tx || prisma;
        const produitIds = lignes
            .map(l => l.produitServiceId)
     ...
     */
    async processReceptionFournisseur(commandeId, lignes, userId, entrepotId, tx) {
        const errors = [];
        const db = tx || prisma;
        const produitIds = lignes
            .map(l => l.produitServiceId)
     ...
     */
    async checkStockAvailability(lignes, entrepotId) {
        ...
        const quantitesDemandees = new Map();
        for (const ligne of lignes) {
            if (!ligne.produitServiceId)
                continue;
     ...
     */
    async transferStock(produitServiceId, quantite, entrepotSourceId, entrepotDestId, userId, motif) {
     ...
     */
    async doInventory(produitServiceId, nouvelleQuantite, entrepotId, userId, motif) {
     ...
     */
    async getProductMouvements(produitServiceId, options) {
        const where = { produitServiceId };
        if (options?.entrepotId)
            where.entrepotId = options.entrepotId;
        if (options?.type)
            where.type = options.type;
        if (options?.dateDebut || options?.dateFin) {
            where.createdAt = {};
            if (options.dateDebut)
                where.createdAt.gte = options.dateDebut;
            if (options.dateFin)
                where.createdAt.lte = options.dateFin;
        }
