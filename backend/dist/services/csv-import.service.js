...
exports.csvService = {
    ...
    async importClients(content) {
        ...
        for (const row of preview.preview) {
            ...
            const existing = await database_js_1.prisma.client.findFirst({
            ...
            if (existing) {
                ...
                const hasSiegeContact = row.siegeContactNom || row.siegeContactFonction || row.siegeContactTel || row.siegeContactEmail;
                if (hasSiegeContact) {
                    await database_js_1.prisma.siegeContact.create({
                        data: {
                ...
                if (hasSiteData) {
                    await database_js_1.prisma.site.create({
                        data: {
                            clientId: existing.id,
                ...
                const createdClient = await database_js_1.prisma.client.create({
                    data: {
    ...
    async previewContrats(content) {
        ...
        const preview = await Promise.all(rows.map(async (row, index) => {
            ...
            if (!row.date_debut || !(0, date_utils_js_1.parseDate)(row.date_debut)) {
     ...
     * Import contrats
     */
    async importContrats(content, userId) {
        ...
        for (const row of preview.preview) {
            if (!row._clientId)
                continue;
            await database_js_1.prisma.contrat.create({
                data: {
                    clientId: row._clientId,
                    type: row.type,
                    dateDebut: (0, date_utils_js_1.parseDate)(row.dateDebut),
                    dateFin: row.dateFin ? (0, date_utils_js_1.parseDate)(row.dateFin) : null,
                    ...
                    premiereDateOperation: row.premiereDateOperation ? (0, date_utils_js_1.parseDate)(row.premiereDateOperation) : null,
                    premiereDateControle: row.premiereDateControle ? (0, date_utils_js_1.parseDate)(row.premiereDateControle) : null,
                    statut: row.statut,
                },
            });
            created++;
     ...
     */
    async previewInterventions(content) {
        const errors = [];
        const rows = this.parseCSV(content);
        const preview = await Promise.all(rows.map(async (row, index) => {
            const rowNum = index + 2;
            ...
            if (!row.date_prevue || !(0, date_utils_js_1.parseDate)(row.date_prevue)) {
     ...
     */
    async importInterventions(content, userId) {
        ...
        for (const row of preview.preview) {
            if (!row._clientId)
                continue;
            await database_js_1.prisma.intervention.create({
                data: {
                    clientId: row._clientId,
                    type: row.type,
                    prestation: row.prestation,
                    datePrevue: (0, date_utils_js_1.parseDate)(row.datePrevue),
     ...
     */
    async exportClients() {
        const clients = await database_js_1.prisma.client.findMany({
     ...
     */
    async exportInterventions(filters) {
        const where = {};
        if (filters?.dateDebut || filters?.dateFin) {
            where.datePrevue = {};
            if (filters.dateDebut)
                where.datePrevue.gte = filters.dateDebut;
            if (filters.dateFin)
                where.datePrevue.lte = filters.dateFin;
        }
        ...
        return (0, sync_2.stringify)(interventions.map(i => ({
...
exports.default = exports.csvService;
