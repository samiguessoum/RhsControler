...
function resolveAttestationLogoPath() {
    const candidates = [
...
function parseSimpleRichText(input) {
    const segments = [];
    let bold = false;
    let underline = false;
    let buffer = '';
...
function drawRichParagraph(doc, text, xStart, yStart, maxWidth, fontSize) {
...
function formatCurrency(amount, devise) {
...
function getStatusLabel(statut, type) {
    const labels = {
        devis: {
            BROUILLON: 'Brouillon',
            VALIDE: 'Validé',
...
function drawHeader(doc, title, ref, date) {
...
function drawClientInfo(doc, client, label = 'Client') {
    const pageWidth = doc.page.width;
    const margin = 50;
    doc.fontSize(10)
        .fillColor(COLORS.lightText)
...
function drawDocumentInfo(doc, infos) {
    const margin = 50;
    let y = 145;
    infos.forEach((info) => {
        doc.fontSize(9)
...
function drawLinesTable(doc, lignes, devise) {
    const margin = 50;
    const pageWidth = doc.page.width;
...
function drawTotals(doc, startY, totalHT, totalTVA, totalTTC, devise, remiseGlobalPct, remiseGlobalMontant, totalPaye) {
    const pageWidth = doc.page.width;
    const margin = 50;
    const boxWidth = 200;
...
function drawFooter(doc, notes, conditions) {
    const margin = 50;
...
function convertUnderHundredFr(num) {
    ...
    if (num < 90)
        return `quatre-vingt-${units[num - 80]}`;
    return `quatre-vingt-${teens[num - 90]}`;
}
function convertUnderThousandFr(num) {
    if (num < 100)
        return convertUnderHundredFr(num);
    const hundreds = Math.floor(num / 100);
    const rest = num % 100;
    const hundredPart = hundreds === 1 ? 'cent' : `${convertUnderHundredFr(hundreds)} cent`;
    if (rest === 0)
        return hundreds > 1 ? `${hundredPart}s` : hundredPart;
...
function amountToWordsDZD(amount) {
...
function drawInvoiceHeader(doc, facture, title) {
    const pageWidth = doc.page.width;
    const margin = 28;
...
function drawLabeledFields(doc, fields, x, startY, width, fontSize = 8, lineHeight = 11) {
    let curY = startY;
...
function buildCompteLabel() {
    return COMPANY_INFO.banque ? `Compte ${COMPANY_INFO.banque}` : 'Compte';
}
function drawInvoiceParties(doc, facture, startY) {
    const margin = 28;
    ...
    drawLabeledFields(doc, [
        ...
        { label: 'NIN', value: COMPANY_INFO.nin },
...
function drawInvoiceLinesTable(doc, facture, startY) {
    const margin = 28;
...
function drawInvoiceTotals(doc, facture, y) {
    const margin = 28;
    const rightW = 220;
...
function drawInvoiceFooter(doc) {
    const margin = 28;
...
function drawDevisHeader(doc, title, ref, date, extraLine, typeLabel) {
    const pageWidth = doc.page.width;
    const margin = 28;
    const logoBoxWidth = 260;
    const rightBoxX = pageWidth - margin - 190;
...
function drawDevisParties(doc, devis, startY) {
    const margin = 28;
    const y = Math.max(104, startY + 10);
    const leftW = 230;
    const rightW = doc.page.width - margin * 2 - leftW - 20;
...
function drawFournisseurParties(doc, fournisseur, startY) {
    const margin = 28;
    const y = Math.max(104, startY + 10);
    const leftW = 230;
    const rightW = doc.page.width - margin * 2 - leftW - 20;
...
function drawDevisLinesTable(doc, lignes, startY) {
    const margin = 28;
    const tableW = doc.page.width - margin * 2;
    const cols = {
        designation: 280,
...
function drawDevisTotals(doc, document, y) {
    const margin = 28;
    const rightW = 220;
    const x = doc.page.width - margin - rightW;
    let curY = y + 4;
...
export async function generateCommandePDF(commande) {
    // Rafraîchir les infos entreprise depuis la DB
    await refreshCompanyInfo();
    return renderStandardDocumentPDF({
...
export async function generateCommandeFournisseurPDF(commande) {
    await refreshCompanyInfo();
...
export async function generateFactureFournisseurPDF(facture) {
    await refreshCompanyInfo();
    const extraLines = [];
    if (facture.refFournisseur)
        extraLines.push(`Réf. fournisseur : ${facture.refFournisseur}`);
    if (facture.dateEcheance)
        extraLines.push(`Échéance : ${formatDate(facture.dateEcheance)}`);
    return renderFournisseurDocumentPDF({
...
export async function generateAttestationPassagePDF(attestation) {
    // Rafraîchir les infos entreprise depuis la DB
    await refreshCompanyInfo();
    return new Promise((resolve, reject) => {
        try {
            ...
            if (attestation.showGuaranteeSection) {
                doc.text(`Les opérations citées ci-dessus sont garanties pour une période de ${attestation.garantieJoursLabel} jours à compter de la date d’exécution des opérations.`, margin, y, { width: contentWidth, align: 'justify', lineGap: 3 });
                y = doc.y + 20;
                doc.text(`La prochaine opération est recommandée pour le ${attestation.dateProchaineOperationFr}.`, margin, y, { width: contentWidth, align: 'justify' });
            }
...
function getStatutBLLabel(statut) {
    const labels = {
        BROUILLON: 'Brouillon',
        CONFIRME: 'Confirmé',
        LIVRE: 'Livré',
...
export async function generateBonLivraisonPDF(bl) {
    await refreshCompanyInfo();
    return new Promise((resolve, reject) => {
        try {
            const doc = new PDFDocument({ size: 'A4', margin: 28 });
