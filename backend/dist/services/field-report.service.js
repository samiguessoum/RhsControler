...
function autosizeColumns(sheet, minWidth = 10) {
...
export const fieldReportService = {
    ...
    async generateSiteHistoryReport(siteId, dateFrom, dateTo, generatedById) {
        ...
        for (const dev of deviceRows) {
            const row = { zone: dev.zoneNom, device: dev.label };
        ...
        const especesByDevice = new Map();
        for (const fi of interventions) {
            for (const ctrl of fi.controls) {
                if (ctrl.device.type !== 'FLYING_INSECT_KILLER')
                    continue;
                if (!especesByDevice.has(ctrl.device.id))
                    especesByDevice.set(ctrl.device.id, new Set());
                for (const ic of ctrl.insectCounts)
                    especesByDevice.get(ctrl.device.id).add(ic.espece);
            }
        }
        for (const dev of fkDevices) {
            const especes = [...(especesByDevice.get(dev.id) || [])].sort();
            for (const espece of especes) {
                const row = { zone: dev.zoneNom, device: dev.label, espece };
                interventions.forEach((fi, i) => {
                    const ctrl = fi.controls.find((c) => c.device.id === dev.id);
...
//# sourceMappingURL=field-report.service.js.map